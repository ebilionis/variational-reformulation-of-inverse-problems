...
# The prior
collection = [UniformND(1), UniformND(1), GammaPDF(5,0.1,1)]
...
results_file = os.path.join('demos', 'diffusion_new_2_cali.pcl')
if os.path.exists(results_file):
    ...
    result['L'] = L
    result['log_q'] = log_q
    with open(os.path.join('demos', 'diffusion_new_2_cali.pcl'), 'wb') as fd:
...
png_file = os.path.join('figures', 'diffusion_new_2_elbo.png')
...
plt.setp(ax.get_xticklabels(), fontsize=16)
plt.setp(ax.get_yticklabels(), fontsize=16)
png_file = os.path.join('figures', 'diffusion_new_2_cali_output.png')
print 'Writing:', png_file
plt.savefig(png_file)

# Do an uncertainty propagation test.
uq_file = os.path.join('demos', 'diffusion_new_2_cali_uq.pcl')
...
plt.setp(ax.get_xticklabels(), fontsize=16)
plt.setp(ax.get_yticklabels(), fontsize=16)
png_file = os.path.join('figures', 'diffusion_new_2_cali_uq.png')
print 'Writing:', png_file
plt.savefig(png_file)
